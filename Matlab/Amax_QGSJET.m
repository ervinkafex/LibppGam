function Amax=Amax_QGSJET(Tp)
% gamma spectra Amax from Pythia8.
% Tp is in GeV
% 
...
% ++++++++++++++++++++++++++++++++++
% 5<=Tp<=100 GeV from Geant4 fit
AHg4=[9.7e-3,-0.35,9.13];
Amax(ihg4) = AHg4(3)*Tpmp(ihg4).^AHg4(2).*exp(AHg4(1)*lTpmp(ihg4).^2);

% ++++++++++++++++++++++++++++++++++
%  Tp>100 GeV from QGSJET fit
AH=[0.01439,-0.4419,13.16];

Amax(ih) = AH(3)*Tpmp(ih).^AH(2).*exp(AH(1)*lTpmp(ih).^2);
% ++++++++++++++++++++++++++++++++++

Amax = Amax.*XSInlcPi0QGSJET(Tp)/mp; %[GeV^-1]
